...
import { LearningReport, ProgressRating } from './classes/entities/learning-report.entity';
...
const MIDDLE_MALE = ['Văn', 'Hữu', 'Minh', 'Đăng', 'Quốc', 'Hoàng', 'Thanh', 'Đức', 'Anh', 'Khánh'];
const MIDDLE_FEMALE = ['Thị', 'Ngọc', 'Quỳnh', 'Thu', 'Như', 'Khánh', 'Minh', 'Thanh', 'Trúc', 'Hồng'];
const FIRST_MALE = ['Bình', 'Đạt', 'Phúc', 'Huy', 'Nam', 'Hùng', 'Kiệt', 'Anh', 'Tú', 'Lộc', 'Sơn', 'Tùng', 'Khoa', 'Lâm', 'Duy', 'Thịnh', 'Khải', 'Hải', 'Phong', 'Quang', 'Cường', 'Dũng', 'Hiếu', 'Long', 'Mạnh'];
const FIRST_FEMALE = ['Cẩm', 'Hà', 'Vy', 'Ngọc', 'Trang', 'Mai', 'Lan', 'Liên', 'Trúc', 'Phương', 'Linh', 'Thảo', 'Yến', 'Anh', 'Chi', 'Diệp', 'Trinh', 'Giang', 'Hương', 'Duyên', 'Nhung', 'Oanh', 'Tuyết', 'Xuân', 'Đào'];

const AREAS = [
  'Quận 1', 'Quận 2', 'Quận 3', 'Quận 4', 'Quận 5', 'Quận 6',
  'Quận 7', 'Quận 8', 'Quận 9', 'Quận 10', 'Quận 11', 'Quận 12',
  'Bình Thạnh', 'Thủ Đức', 'Gò Vấp', 'Tân Bình', 'Tân Phú', 'Phú Nhuận',
  'Cầu Giấy, Hà Nội', 'Hải Châu, Đà Nẵng',
];

const EDU_LEVELS = ['Sinh viên', 'Cử nhân', 'Giáo viên', 'Thạc sĩ'];
const MAJORS = ['Sư phạm Toán', 'Sư phạm Vật lí', 'Sư phạm Hóa', 'Ngôn ngữ Anh', 'Công nghệ thông tin', 'Toán tin', 'Khoa học máy tính', 'Sư phạm Ngữ văn'];
const UNIVERSITIES = ['Đại học Sư phạm TP.HCM', 'Đại học Bách Khoa TP.HCM', 'Đại học Khoa học Tự nhiên TP.HCM', 'Đại học Sài Gòn', 'Đại học Sư phạm Hà Nội', 'Đại học Ngoại ngữ - ĐHQGHN', 'Đại học Sư phạm Đà Nẵng'];
...
const REVIEW_COMMENTS = [
  { min: 1, max: 2, comments: ['Gia sư cần cải thiện phương pháp giảng dạy.', 'Chưa đáp ứng được yêu cầu học tập.', 'Cần chuẩn bị bài kỹ hơn trước mỗi buổi học.'] },
  { min: 3, max: 3, comments: ['Tạm ổn, gia sư nhiệt tình nhưng cần chuyên nghiệp hơn.', 'Có tiến bộ nhưng chưa như kỳ vọng.', 'Gia sư đúng giờ, kiến thức còn hạn chế ở một số phần.'] },
...
function getRandomReviewComment(): { rating: number; comment: string } {
  const templates = REVIEW_COMMENTS[Math.floor(Math.random() * REVIEW_COMMENTS.length)];
  const rating = Math.floor(Math.random() * (templates.max - templates.min + 1)) + templates.min;
...
async function seed() {
  ...
  for (const name of roleNames) {
    ...
    if (!role) {
      role = await roleRepo.save(roleRepo.create({ name, description: `Vai trò ${name}` }));
  ...
  for (const name of subjectNames) {
    ...
    if (!subject) {
      subject = await subjectRepo.save(subjectRepo.create({ name, isActive: true }));
  ...
  const adminData = { email: 'admin@tutoredu.com', fullName: 'Hệ thống Admin', password: 'Admin123@' };
  const staffData = { email: 'staff@tutoredu.com', fullName: 'Nhân viên quản lý', password: 'Staff123@' };
  ...
  const hardcodedTutors = [
    { email: 'nguyen_van_binh@tutoredu.com', fullName: 'Nguyễn Văn Bình', phone: '0901222333', address: 'Quận 1, TP.HCM', avatarUrl: 'https://storage.googleapis.com/banani-avatars/avatar%2Fmale%2F25-35%2FSoutheast%20Asian%2F1', tutor: { educationLevel: 'Sinh viên', major: 'Khoa học máy tính', university: 'Đại học Bách Khoa TP.HCM', experience: '2 năm kinh nghiệm dạy kèm môn Toán, Lý cấp 3', idCardNumber: '079100000001', availableAreas: 'Quận 1, Quận 5', approvalStatus: ApprovalStatus.APPROVED }, subjects: ['Toán học', 'Vật lí'] },
    { email: 'tran_thi_cam@tutoredu.com', fullName: 'Trần Thị Cẩm', phone: '0902222333', address: 'Quận 7, TP.HCM', avatarUrl: 'https://storage.googleapis.com/banani-avatars/avatar%2Ffemale%2F25-35%2FSoutheast%20Asian%2F2', tutor: { educationLevel: 'Giáo viên', major: 'Sư phạm Vật lí', university: 'Đại học Sư phạm TP.HCM', experience: '5 năm kinh nghiệm giảng dạy môn Toán và Hóa học', idCardNumber: '079100000002', availableAreas: 'Quận 1, Quận 4, Quận 7', approvalStatus: ApprovalStatus.APPROVED }, subjects: ['Toán học', 'Hóa học'] },
    { email: 'le_minh_dat@tutoredu.com', fullName: 'Lê Minh Đạt', phone: '0903222333', address: 'Quận 10, TP.HCM', avatarUrl: 'https://storage.googleapis.com/banani-avatars/avatar%2Fmale%2F18-25%2FSoutheast%20Asian%2F2', tutor: { educationLevel: 'Sinh viên', major: 'Công nghệ thông tin', university: 'Đại học Khoa học Tự nhiên TP.HCM', experience: '1.5 năm kinh nghiệm, rành kèm học sinh trung học', idCardNumber: '079100000003', availableAreas: 'Quận 3, Quận 10', approvalStatus: ApprovalStatus.PENDING }, subjects: ['Toán học', 'Tin học'] },
    { email: 'phan_ngoc_ha@tutoredu.com', fullName: 'Phan Ngọc Hà', phone: '0904222333', address: 'Bình Thạnh, TP.HCM', avatarUrl: 'https://storage.googleapis.com/banani-avatars/avatar%2Ffemale%2F25-35%2FSoutheast%20Asian%2F1', tutor: { educationLevel: 'Cử nhân', major: 'Sư phạm Toán', university: 'Đại học Sư phạm TP.HCM', experience: '3 năm kinh nghiệm dạy ôn thi', idCardNumber: '079100000004', availableAreas: 'Bình Thạnh', approvalStatus: ApprovalStatus.APPROVED }, subjects: ['Toán học'] },
    { email: 'nguyen_minh_anh@tutoredu.com', fullName: 'Nguyễn Minh Anh', phone: '0901234567', address: 'Quận 1, TP.HCM', avatarUrl: 'https://storage.googleapis.com/banani-avatars/avatar%2Ffemale%2F25-35%2FSoutheast%20Asian%2F4', tutor: { educationLevel: 'Tốt nghiệp', major: 'Kỹ thuật điện', university: 'Đại học Bách Khoa TP.HCM', experience: '5 năm kinh nghiệm dạy kèm Toán THPT, luyện thi Đại học', idCardNumber: '079100000005', availableAreas: 'Quận 1, TP.HCM', approvalStatus: ApprovalStatus.APPROVED }, subjects: ['Toán học'] },
    { email: 'tran_hoang_phuc@tutoredu.com', fullName: 'Trần Hoàng Phúc', phone: '0912345678', address: 'Quận 3, TP.HCM', avatarUrl: 'https://storage.googleapis.com/banani-avatars/avatar%2Fmale%2F25-35%2FSoutheast%20Asian%2F3', tutor: { educationLevel: 'Thạc sĩ', major: 'Toán ứng dụng', university: 'Đại học Khoa học Tự nhiên TP.HCM', experience: '4 năm kinh nghiệm dạy kèm Toán lớp 10-12, luyện thi', idCardNumber: '079100000006', availableAreas: 'Quận 3, TP.HCM', approvalStatus: ApprovalStatus.APPROVED }, subjects: ['Toán học'] },
    { email: 'le_khanh_vy@tutoredu.com', fullName: 'Lê Khánh Vy', phone: '0923456789', address: 'Bình Thạnh, TP.HCM', avatarUrl: 'https://storage.googleapis.com/banani-avatars/avatar%2Ffemale%2F18-25%2FSoutheast%20Asian%2F5', tutor: { educationLevel: 'Sinh viên', major: 'Sư phạm Toán', university: 'Đại học Sư phạm TP.HCM', experience: '3 năm kinh nghiệm dạy kèm Toán cơ bản và Toán cấp 2', idCardNumber: '079100000007', availableAreas: 'Bình Thạnh', approvalStatus: ApprovalStatus.APPROVED }, subjects: ['Toán học'] },
    { email: 'pham_quoc_huy@tutoredu.com', fullName: 'Phạm Quốc Huy', phone: '0934567890', address: 'Thủ Đức, TP.HCM', avatarUrl: 'https://storage.googleapis.com/banani-avatars/avatar%2Fmale%2F18-25%2FSoutheast%20Asian%2F1', tutor: { educationLevel: 'Sinh viên', major: 'Toán tin', university: 'Đại học Khoa học Tự nhiên TP.HCM', experience: '2 năm kinh nghiệm dạy kèm 1-1 Toán THPT', idCardNumber: '079100000008', availableAreas: 'Thủ Đức', approvalStatus: ApprovalStatus.APPROVED }, subjects: ['Toán học'] },
    { email: 'nguyen_tran_bao_ngoc@tutoredu.com', fullName: 'Nguyễn Trần Bảo Ngọc', phone: '0905222333', address: 'Cầu Giấy, Hà Nội', avatarUrl: 'https://storage.googleapis.com/banani-avatars/avatar%2Ffemale%2F18-25%2FSoutheast%20Asian%2F1', tutor: { educationLevel: 'Sinh viên', major: 'Sư phạm Toán', university: 'Đại học Sư phạm Hà Nội', experience: '2 năm kinh nghiệm dạy kèm Toán cấp 3, luyện thi Đại học.', idCardNumber: '079100000009', availableAreas: 'Cầu Giấy, Đống Đa, Ba Đình', approvalStatus: ApprovalStatus.APPROVED }, subjects: ['Toán học'] },
    { email: 'le_hoang_nam@tutoredu.com', fullName: 'Lê Hoàng Nam', phone: '0906222333', address: 'Quận 1, TP.HCM', avatarUrl: 'https://storage.googleapis.com/banani-avatars/avatar%2Fmale%2F25-35%2FSoutheast%20Asian%2F2', tutor: { educationLevel: 'Cử nhân', major: 'Ngôn ngữ Anh', university: 'Đại học Ngoại ngữ - ĐHQGHN', experience: 'Cử nhân Ngôn Ngữ Anh, 4 năm kinh nghiệm giảng dạy tiếng Anh giao tiếp và luyện thi IELTS 7.5+.', idCardNumber: '079100000010', availableAreas: 'Quận 1, Quận 3, Quận 7', approvalStatus: ApprovalStatus.APPROVED }, subjects: ['Tiếng Anh'] },
    { email: 'tran_thi_mai@tutoredu.com', fullName: 'Trần Thị Mai', phone: '0907222333', address: 'Hải Châu, Đà Nẵng', avatarUrl: 'https://storage.googleapis.com/banani-avatars/avatar%2Ffemale%2F25-35%2FSoutheast%20Asian%2F3', tutor: { educationLevel: 'Giáo viên', major: 'Giáo dục Tiểu học', university: 'Đại học Sư phạm Đà Nẵng', experience: '7 năm kinh nghiệm giảng dạy tiểu học.', idCardNumber: '079100000011', availableAreas: 'Hải Châu, Thanh Khê, Sơn Trà', approvalStatus: ApprovalStatus.APPROVED }, subjects: ['Toán học', 'Ngữ văn'] },
  ];
  ...
  for (let i = 1; i <= extraTutorCount; i++) {
    ...
    const middleName = isMale ? pickRandom(MIDDLE_MALE) : pickRandom(MIDDLE_FEMALE);
    const firstName = isMale ? pickRandom(FIRST_MALE) : pickRandom(FIRST_FEMALE);
    const fullName = `${lastName} ${middleName} ${firstName}`;
    const emailSlug = removeVietnameseTones(fullName.toLowerCase()).replace(/\s+/g, '_');
    ...
    allTutorData.push({
      ...
      tutor: {
        ...
        availableAreas: area,
        approvalStatus: i % 8 === 0 ? ApprovalStatus.PENDING : ApprovalStatus.APPROVED,
      },
  ...
  for (const item of allTutorData) {
    ...
    if (existingUser) {
      ...
      let existingTutor = await tutorRepo.findOne({ where: { user: { id: existingUser.id } } });
      if (existingTutor) {
        ...
        existingTutor.approvalStatus = item.tutor.approvalStatus as ApprovalStatus;
        ...
        existingTutor = await tutorRepo.save(tutorRepo.create({
          ...item.tutor,
          user: existingUser,
        }));
        seededTutorsMap.set(item.email, existingTutor);
  ...
  const hardcodedStudents = [
    { email: 'le_trong_tan@tutoredu.com', fullName: 'Lê Trọng Tấn', phone: '0901234567', gradeLevel: 'Lớp 10', parentName: 'Lê Trọng Tấn' },
    { email: 'ngo_minh_an@tutoredu.com', fullName: 'Ngô Minh An', phone: '0935451882', gradeLevel: 'Lớp 12', parentName: 'Ngô Minh An' },
    { email: 'tran_my_duyen@tutoredu.com', fullName: 'Trần Mỹ Duyên', phone: '0918877520', gradeLevel: 'Lớp 11', parentName: 'Trần Mỹ Duyên' },
    { email: 'pham_huu_duc@tutoredu.com', fullName: 'Phạm Hữu Đức', phone: '0982612008', gradeLevel: 'Lớp 9', parentName: 'Phạm Hữu Đức' },
    { email: 'hoang_gia_bao@tutoredu.com', fullName: 'Hoàng Gia Bảo', phone: '0907225114', gradeLevel: 'Lớp 12', parentName: 'Hoàng Gia Bảo' },
    { email: 'nguyen_thi_ha@tutoredu.com', fullName: 'Nguyễn Thị Hà', phone: '0908888999', gradeLevel: 'Lớp 11', parentName: 'Nguyễn Thị Hà' },
  ];
  ...
  const requestStatuses = [RequestStatus.PENDING, RequestStatus.PROCESSING, RequestStatus.MATCHED, RequestStatus.CANCELLED];
  const schedules = ['Tối Thứ 2, Thứ 4 · 19:00 - 21:00', 'Sáng Thứ 7, Chủ nhật · 08:00 - 10:00', 'Chiều Thứ 3, Thứ 5 · 17:30 - 19:30', 'Tối Thứ 2, Thứ 5 · 18:00 - 20:00', 'Sáng Thứ 3, Thứ 6 · 09:00 - 11:00', 'Chiều Thứ 4, Thứ 7 · 14:00 - 16:00'];
  const requirements = [
  ...
  const studentsForRequests = allStudentEmails.sort(() => Math.random() - 0.5).slice(0, 120);
  ...
  const matchableRequests = await requestRepo.find({
    where: [
      { status: RequestStatus.MATCHED as any },
      { status: RequestStatus.PROCESSING as any },
  ...
  const approvedTutors = await tutorRepo.find({
    where: { approvalStatus: ApprovalStatus.APPROVED as any },
    relations: ['user'],
    take: 60,
  });
  ...
  const hardcodedClassMappings = [
    { tutorEmail: 'tran_thi_cam@tutoredu.com', studentEmail: 'tran_my_duyen@tutoredu.com', subject: chemSubject, fee: 300000, total: 20, status: 'active' as any, location: 'Quận 7, TP.HCM', startDate: new Date('2026-05-01') },
    { tutorEmail: 'tran_thi_cam@tutoredu.com', studentEmail: 'nguyen_thi_ha@tutoredu.com', subject: mathSubject, fee: 250000, total: 15, status: 'active' as any, location: 'Quận 7, TP.HCM', startDate: new Date('2026-06-01') },
    { tutorEmail: 'nguyen_van_binh@tutoredu.com', studentEmail: 'le_trong_tan@tutoredu.com', subject: mathSubject, fee: 250000, total: 10, status: 'completed' as any, location: 'Quận 1, TP.HCM', startDate: new Date('2026-04-01'), endDate: new Date('2026-05-01') },
    { tutorEmail: 'phan_ngoc_ha@tutoredu.com', studentEmail: 'tran_my_duyen@tutoredu.com', subject: mathSubject, fee: 200000, total: 15, status: 'active' as any, location: 'Bình Thạnh, TP.HCM', startDate: new Date('2026-05-01') },
    { tutorEmail: 'nguyen_minh_anh@tutoredu.com', studentEmail: 'le_trong_tan@tutoredu.com', subject: mathSubject, fee: 250000, total: 15, status: 'active' as any, location: 'Quận 1, TP.HCM', startDate: new Date('2026-05-01') },
    { tutorEmail: 'tran_hoang_phuc@tutoredu.com', studentEmail: 'hoang_gia_bao@tutoredu.com', subject: mathSubject, fee: 300000, total: 15, status: 'active' as any, location: 'Quận 3, TP.HCM', startDate: new Date('2026-05-01') },
    { tutorEmail: 'le_khanh_vy@tutoredu.com', studentEmail: 'ngo_minh_an@tutoredu.com', subject: mathSubject, fee: 180000, total: 15, status: 'active' as any, location: 'Bình Thạnh, TP.HCM', startDate: new Date('2026-05-01') },
    { tutorEmail: 'pham_quoc_huy@tutoredu.com', studentEmail: 'pham_huu_duc@tutoredu.com', subject: mathSubject, fee: 220000, total: 15, status: 'active' as any, location: 'Thủ Đức, TP.HCM', startDate: new Date('2026-05-01') },
    { tutorEmail: 'nguyen_tran_bao_ngoc@tutoredu.com', studentEmail: 'nguyen_thi_ha@tutoredu.com', subject: mathSubject, fee: 250000, total: 15, status: 'active' as any, location: 'Cầu Giấy, Hà Nội', startDate: new Date('2026-05-01') },
    { tutorEmail: 'le_hoang_nam@tutoredu.com', studentEmail: 'ngo_minh_an@tutoredu.com', subject: engSubject, fee: 350000, total: 15, status: 'active' as any, location: 'Quận 1, TP.HCM', startDate: new Date('2026-05-01') },
    { tutorEmail: 'tran_thi_mai@tutoredu.com', studentEmail: 'pham_huu_duc@tutoredu.com', subject: litSubject, fee: 200000, total: 15, status: 'active' as any, location: 'Hải Châu, Đà Nẵng', startDate: new Date('2026-05-01') },
  ];
  ...
  for (const mapping of hardcodedClassMappings) {
    ...
    if (!existing) {
      const newClass = await classRepo.save(classRepo.create({
        tutor,
        student,
  ...
  const usersForNotifications = await userRepo.find({ take: 60, order: { createdAt: 'ASC' } });

  for (const user of usersForNotifications) {
    if (user.role.name === 'admin') continue; // Skip admin, they get too many
    const existingNotif = await notificationRepo.findOneBy({ user: { id: user.id } });
